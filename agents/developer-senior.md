...
- **Design decisions** — when a design decision genuinely emerges during implementation (spec doesn't cover it), return the decision as a note in `report.json` (`design_decision`) — the orchestrator Logs it. You do not write the spec Log (or any spec / `exec.md` / `observed` / checkoff / status), and you do not spawn the compliance-checker; see the allowlist in `skills/feature/references/developer-workflow.md` §What you write. Future-you and reviewers need the reasoning, so make the note explicit.
