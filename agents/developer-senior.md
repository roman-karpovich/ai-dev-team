...
6. **For each step** (in order):
   ...
   h. Commit the changes (small logical commit per step)
   i. Mark the checkbox `[x]` in the spec
   j. **Spawn `spec-compliance-checker`** subagent with: `spec_path`, `workdoc_path`, `step_number`, `project_path`
   k. If compliance result is FAIL or DRIFT: address all listed issues, re-run captures, re-commit, re-run checker
