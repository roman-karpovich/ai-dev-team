...
- **codex_model** (optional): override the Codex model (e.g. `gpt-5.6-sol`). Populated from `.ai-dev-team.yml` under `codex.model`. If absent, leave the model unset (empty `CODEX_MODEL` for the CLI dispatch helper / omitted `model` field in MCP calls) so Codex resolves it from `~/.codex/config.toml` — the single global model config.
