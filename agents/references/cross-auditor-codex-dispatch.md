...
- Codebase-grounded verification. For every concrete code referent the spec ASSERTS — a file path, symbol/function name, constant/threshold value, config key, data-structure field, or claim about existing behavior — OPEN the cited code and confirm it exists with the claimed name, value, and semantics. Named file/symbol/constant/config key absent from the code → HIGH (the design rests on a referent that isn't there). Referent present but value/semantics DIFFER from the spec's claim (e.g. spec assumes VOTING_PERIOD = 7 days, code says 10; spec assumes a function returns a sorted list, code does not) → HIGH if the design depends on the differing value/semantics, else MEDIUM.
