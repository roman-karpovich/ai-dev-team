...
- **Derive expected values from test inputs, not magic constants**: if the expected value follows logically from the test setup, express it that way — `assert_eq!(reserve, deposit1 + deposit2)` rather than `assert_eq!(reserve, 500001000)`. When the value can't be derived trivially, compute it in the test using the same formula the production code should follow — assign it to a named variable and assert against that. This makes the test a parallel independent implementation of the logic: if the formula changes, both break, which is the point. Raw numeric constants are a last resort, only when no formula can be expressed (e.g. a precomputed hash or an opaque external value)
