...
- **Derive expected values from test inputs, not magic constants**: if the expected value follows logically from the test setup, express it that way — `assert_eq!(reserve, deposit1 + deposit2)` rather than `assert_eq!(reserve, 500001000)`. Magic numbers hide the reasoning chain and make tests hard to understand and maintain. Raw constants are acceptable only when the value cannot be cleanly derived (e.g., a swap result involving AMM math)
