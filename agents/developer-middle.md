...
5. **Read relevant source files** before writing. Match style exactly. If the step involves writing a test: read 2-3 existing tests in the same file or directory first — match their structure, naming, fixtures, and assertion style exactly. Prefer exact assertions over vague ones (`assert_eq!(x, 42)` not `assert!(x > 0)`). Derive expected values from test inputs rather than magic constants — `assert_eq!(reserve, deposit1 + deposit2)` not `assert_eq!(reserve, 500001000)`. When the value can't be derived trivially, compute it in the test using the same formula production code should follow (assign to a named variable, assert against it) — the test becomes a parallel implementation of the logic. Raw numeric constants only when no formula can be expressed (precomputed hash, opaque external value). Fix non-determinism — freeze dates/times (freezegun, MockClock, jest.useFakeTimers, etc.), seed random values. A test that can break on a Friday or after a year is a time bomb, not a test.
