...
resume. Trigger IDs below (`T-C#`, `T-S#`, `T-CF#`) are the only valid `rationale=` values in `last_agent=` Log entries.
...
**Triggers**:
- **T-S0**: no Codex trigger and no Codex Fast trigger matched (fallback)
...
- spec is fully prompt-specifiable and a pattern already exists (prefer Codex or Codex Fast)
- task is a trivial one-liner (prefer Codex or Codex Fast)
...
`rationale=` MUST be a trigger ID from one of the three agent sections above (including `Codex Fast`).
Use `T-S0` when none of Codex's or Codex Fast's triggers matched (the fallback).
