...
5. **Continue-gate — blocks on branch AND HEAD.** NEVER continue the per-step loop / Log / checkoff while EITHER the branch != `pre_spawn_branch` OR the callsite's step-3 HEAD condition is unsatisfied. The gate blocks on BOTH conditions, not branch alone — otherwise a branch-correct / HEAD-wrong state (the exact false-green this guard prevents) slips through.
