#!/usr/bin/env bash
# probe_g.sh — Probe G — dependency freshness.
#
# stdin envelope:
#   {
...
#   }
#
# Radaro AI-Assisted Development Policy v1.3 §8.3 calls out major-version
# drift as a dependency-layer supply-chain risk class.
#
# Determinism seam: when PROBE_G_FAKE_NOW env var is set, receipt_metadata's
...
STDIN_PAYLOAD=$(cat)
PROBE_G_SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"

STDIN_PAYLOAD="$STDIN_PAYLOAD" PROBE_G_SCRIPT_DIR="$PROBE_G_SCRIPT_DIR" python3 <<'PY'
import hashlib
import json
import os
import re
...
def die(msg, code=2):
    sys.stderr.write(f"probe_g.sh: {msg}\n")
    sys.exit(code)


def emitted_at_now():
    emitted_at = os.environ.get("PROBE_G_FAKE_NOW")
    if not emitted_at:
...
def load_corpus():
    ...
    if plugin_root:
        ...
        corpus_path = os.path.join(
            os.environ.get("PROBE_G_SCRIPT_DIR", "."),
            "freshness_corpus.json"
        )
    try:
...
def corpus_package_count(corpus):
    total = 0
    for packages in corpus.values():
...
def parse_major(version):
    head = version.split(".", 1)[0].lstrip("v")
    return int(head)
...
corpus = load_corpus()
package_count = corpus_package_count(corpus)
scope_files_read = []
skipped_files = []
findings = []
seen_findings = set()
...
for rel, abs_path, name in lockfiles:
    ...
    for ecosystem, package, version in pinned:
        try:
            pinned_major = parse_major(version)
        ...
        if not isinstance(latest_major, int):
            continue
        if pinned_major >= latest_major - 2:
            continue
        key = (ecosystem, package, rel)
        if key in seen_findings:
            continue
        seen_findings.add(key)
        drift = latest_major - pinned_major
        findings.append({
            "provisional_id": f"g-{ecosystem}-{package}",
            "sources": [f"probe:{PROBE_ID}"],
            "severity": "HIGH",
            "title": (
                f"Dependency {package} is {drift} major versions behind "
                "latest stable"
            ),
            "file": rel,
            "description": (
                f"Pinned major v{pinned_major}; latest stable v{latest_major}. "
                "Major-version drift >2 is a likely security-update gap."
            ),
            "fix": (
                f"Bump {package} from v{pinned_major}.x to v{latest_major}.x "
                f"in {rel}; review breaking-change notes; run test suite."
            ),
            "fingerprint_anchors": {
                "package": package,
                "ecosystem": ecosystem,
...
eligible_reason = (
    ...
    f"{package_count} packages in corpus; {len(findings)} findings"
)
