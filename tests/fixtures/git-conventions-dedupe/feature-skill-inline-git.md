...
- **Feature branch**: `<type>/YYYY-MM-DD-<slug>` — `<type>` is the spec's resolved `change_type` (one of `feat / fix / refactor / ci / docs / test / chore`); dated example: `feat/2026-04-17-my-feature` (or as specified in spec `Branch:` field; see CLAUDE.md §Contribution flow)
